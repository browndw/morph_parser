...
Starting with version 0.2.0, morph_parser uses lightweight, custom implementations for statistical computations instead of heavy dependencies like scipy and statsmodels. This design choice reduces installation overhead while maintaining full functionality for the core VNC methodology and smoothing operations.
...
You can install the released version of morph_parser from [PyPI](https://pypi.org/project/google-ngrams/){.external target="_blank"}:
...
Data retrieved from the [](`~morph_parser.ngrams.google_ngram`) function (or any other time series data) can then be analyzed using `TimeSeries`.
...
```{python}
quiz_ts.timeviz_barplot(width=4, height=2, dpi=200);
```
