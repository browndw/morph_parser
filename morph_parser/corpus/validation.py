"""
Enhanced error handling and validation utilities for morph_parser.
...
Exception Classes:
    CorpusProcessingError: Base exception for all corpus-processing errors
...
Example:
    Using validation functions::
        ...
        import polars as pl
        from morph_parser.corpus.validation import validate_corpus_dataframe
    ...
    Custom error handling::
        ...
        from morph_parser.corpus.validation import CorpusProcessingError
        from morph_parser.corpus.parse_utils import CorpusProcessor

        try:
            # Your morph_parser operations
            result = CorpusProcessor().process_corpus(...)
        except CorpusProcessingError as e:
            # Handle all corpus-specific errors
            print(f"Corpus processing error: {e}")
...
class CorpusProcessingError(Exception):
    """
    Base exception class for all corpus-processing errors.

    This is the parent class for all custom exceptions in the corpus module.
    It allows for catching all corpus-specific errors with
    a single except clause.

    Example:
        Catch all corpus-processing errors::

            from morph_parser.corpus.validation import CorpusProcessingError

            try:
                # Your corpus operations
                result = CorpusProcessor().process_corpus(...)
            except CorpusProcessingError as e:
...
class CorpusValidationError(CorpusProcessingError):
    """
    ...
    Example:
        >>> import polars as pl
            >>> from morph_parser.corpus.validation import (
...
class ModelValidationError(CorpusProcessingError):
    """Raised when spaCy model validation fails."""

    pass


class DataFormatError(CorpusProcessingError):
    """Raised when data format is incorrect."""

    pass


class ParameterValidationError(CorpusProcessingError):
    """Raised when function parameters are invalid."""

    pass


class FileSystemError(CorpusProcessingError):
...
def validate_tokens_dataframe(
    ...
    # Expected schemas for token-level output produced by
    # CorpusProcessor().process_corpus().
    expected_schema_with_lemma = OrderedDict(
        [
            ("doc_id", pl.String),
    ...
    expected_schema_without_lemma = OrderedDict(
        [
            ("doc_id", pl.String),
            ("sentence_id", pl.UInt32),
            ("token_id", pl.Int64),
    ...
    if (
        actual_schema != expected_schema_with_lemma
        and actual_schema != expected_schema_without_lemma
    ):
        expected_keys = set(expected_schema_with_lemma.keys())
        missing_cols = expected_keys - set(actual_schema.keys())
        extra_cols = set(actual_schema.keys()) - expected_keys
        wrong_types = {
            col: (expected_schema_with_lemma[col], actual_schema.get(col))
            for col in expected_schema_with_lemma
            if (
                col in actual_schema
                and actual_schema[col] != expected_schema_with_lemma[col]
            )
        }
        ...
        error_msg += (
            ...
            "token (String), [optional lemma (String)], pos (String), "
            "tag (String), head_token_id (Int64), dep_rel (String), "
            "morph (String)"
        )
...
def validate_spacy_model(nlp_model: Language, context: str = "") -> None:
    """Validate that a spaCy model is compatible with this corpus module.
...
# Backward-compatible alias while old notebooks migrate naming.
PybiberError = CorpusProcessingError
