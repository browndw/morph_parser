...
class TextPreprocessor:
    ...
    Example:
        Use individual preprocessing methods::
            ...
            from morph_parser.corpus.parse_utils import TextPreprocessor
    ...
    @staticmethod
    def strip_editorial_trailing_hyphens(text: str) -> str:
        """
        Remove trailing editorial hyphen runs attached to word forms.

        Historical corpora sometimes encode interruptions or line-end artifacts
        as repeated ASCII hyphens immediately after a word, e.g. ``dog--`` or
        ``Martial--``. These should not survive into the token stream as part
        of the lexical item. Internal repeated hyphens between alphabetic spans
        are preserved.

        Args:
            text: Input text that may contain editorial hyphen artifacts.

        Returns:
            Text with trailing editorial hyphen runs removed.
        """
        return re.sub(r"(?<=[A-Za-z])-{2,}(?=[^A-Za-z]|$)", " ", text)
    ...
    def preprocess_corpus(corp: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            ...
            .with_columns(
                pl.col("text").map_elements(
                    TextPreprocessor.strip_editorial_trailing_hyphens,
                    return_dtype=pl.String,
                )
            )
            .with_columns(
...
class TextChunker:
    ...
    Example:
        Split a long document::
            ...
            from morph_parser.corpus.parse_utils import TextChunker
...
class SpacyProcessor:
    ...
    def process_with_spacy(
        ...
        for doc, context in doc_tuples:
            ...
            morph_list = [str(token.morph) for token in doc]
...
class DataFrameTransformer:
    ...
    def transform_spacy_output(
        ...
        original_doc_order: List[str] = None,
...
class CorpusProcessor:
    ...
    def process_corpus(
        ...
        :param lemma: Whether to include a lemma column in token output.
        ...
        with PerformanceMonitor("Corpus processing"):
            ...
            try:
                ...
                result = self.transformer.transform_spacy_output(
                    df_list,
                    original_doc_order=original_doc_order,
                    include_lemma=lemma,
                )
                if show_progress:
                    progress.update(len(corp) // 4)
